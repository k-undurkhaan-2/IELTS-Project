...
test('docker image hardening excludes secrets and runs app as non-root', () => {
    ...
    const compose = fs.readFileSync(path.join(repoRoot, 'backend', 'docker-compose.yml'), 'utf8').replace(/\r\n?/g, '\n');
    ...
    assert(!envExample.includes('ALLOW_LEGACY_DIRECT_ACCOUNT_APIS'));
    assert(!compose.includes('ALLOW_LEGACY_DIRECT_ACCOUNT_APIS'));
    ...
    assert(!deploymentRunbook.includes('ALLOW_LEGACY_DIRECT_ACCOUNT_APIS'));
    assert(deploymentRunbook.includes('Legacy direct-app account and TOTP-disable mutation APIs are retired.'));
    assert.match(deploymentRunbook, /Do not\s+reintroduce opt-in environment switches/);
...
test('admin shell and business account menu do not link back through the business home', () => {
    ...
    assert.doesNotMatch(authOverlay, /loadTotpPanel|remote-auth-totp|disableTotp|\/api\/auth\/totp\/disable|\/api\/auth\/account/);
    assert.doesNotMatch(remoteApiClient, /disableTotp|\/api\/auth\/totp\/disable|\/api\/auth\/account/);
    ...
    assert(!fs.existsSync(path.join(repoRoot, 'backend', 'auth', 'account.html')));
    assert(!fs.existsSync(path.join(repoRoot, 'backend', 'auth', 'account.js')));
    assert(!fs.existsSync(path.join(repoRoot, 'backend', 'auth', 'account.css')));
    ...
    const restoreBackupWrite = bootFallbacks.indexOf('await manager.restoreBackup(backupId)', restoreBackupStart);
    assert(restoreBackupStart >= 0);
    assert(restoreBackupStepUp > restoreBackupStart);
    assert(restoreBackupStepUp < restoreBackupConfirm);
    assert(restoreBackupStepUp < restoreBackupWrite);
    const exportDataStart = bootFallbacks.indexOf('window.exportAllData = async function');
    ...
    assert(createBackupEnd > createBackupStart);
    assert(!bootFallbacks.slice(createBackupStart, createBackupEnd).includes('ensureFallbackDataManageStepUp'));
